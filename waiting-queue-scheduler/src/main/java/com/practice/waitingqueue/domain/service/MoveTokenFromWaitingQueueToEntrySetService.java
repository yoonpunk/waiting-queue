...
public class MoveTokenFromWaitingQueueToEntrySetService {
    ...
    private static final int DEFAULT_TOKEN_COUNT_TO_MOVE = 1;
     ...
     * 기본값은 1개이며, 설정을 통해 변경할 수 있습니다.
     * 0개일 때는 대기열에서 토큰을 이동하지 않습니다. (잠시 입장셋에 진입을 막는 용도로 사용)
     */
    ...
    public void moveWaitingQueueTokens() {
        ...
        var totalMovedTokenCount = 0;
        for (final var itemId : queueingItemIdList) {
            ...
            totalMovedTokenCount += waitingQueueList.size();
            log.info("[MoveTokenFromWaitingQueueToEntrySetService] enter entry set: itemId={} tokenCount={}", itemId, waitingQueueList.size());
        }

        log.info("[MoveTokenFromWaitingQueueToEntrySetService] end moving tokens from waiting queue to entry set. totalMovedTokenCount={}", totalMovedTokenCount);
    }
}
